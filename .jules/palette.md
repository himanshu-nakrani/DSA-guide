...
## 2026-08-21 - Avoid duplicate ARIA attributes on a single element
**Learning:** Duplicate ARIA attributes (e.g., multiple `aria-expanded` props) on a single element (such as the toggle buttons in `Sidebar.tsx`) are invalid React JSX syntax, cause linter errors, and can lead to unpredictable parsing behavior for assistive technologies like screen readers.
**Action:** Always ensure that React components have unique props. Carefully check for duplicate `aria-*` attributes (such as `aria-expanded`, `aria-label`, etc.) and ensure only a single valid instance is present per element.
