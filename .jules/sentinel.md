...
## 2026-09-14 - Missing Security Headers
**Vulnerability:** The application was missing basic security headers, leaving it vulnerable to common browser-based attacks such as clickjacking (lack of X-Frame-Options), MIME-type sniffing (lack of X-Content-Type-Options), and potentially downgrade attacks or XSS.
**Learning:** Modern web applications require defense-in-depth protections provided by HTTP security headers to mitigate a variety of client-side vulnerabilities, even if the application logic itself is secure. Next.js does not apply these strict headers by default.
**Prevention:** Always configure Next.js (`next.config.ts`) to return an array of strict HTTP security headers (e.g., `Strict-Transport-Security`, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `X-DNS-Prefetch-Control: on`, and `X-XSS-Protection: 0`) applied to all paths (`/:path*`) to provide defense-in-depth against browser-level exploits.

## 2026-09-21 - Unprotected State-Modifying Server Actions (Rate Limiting and Input Constraints)
**Vulnerability:** Several state-modifying server actions for lists (`renameListAction`, `deleteListAction`, `toggleListPublicAction`) lacked both rate limiting and strict maximum length bounds on inputs. This left the system vulnerable to Denial of Service (DoS) attacks and resource exhaustion via unchecked database writes.
**Learning:** All state-modifying Server Actions, regardless of how obscure or rarely used they appear, must be protected by rate limiters and enforce strict validation on string inputs before processing or database insertion. Failing to secure secondary actions provides an attacker with a bypass to quota limitations.
**Prevention:** Always ensure that `checkRateLimit` (scoped to the authenticated `user.id`) is called at the very beginning of state-modifying actions. Additionally, always explicitly enforce maximum string lengths (e.g., `length > 255`) on fields before calling database write methods (e.g., `Prisma.update`).
