...
## 2024-07-28 - Missing Rate Limiting on State-Modifying Actions
**Vulnerability:** The `renameListAction`, `deleteListAction`, and `toggleListPublicAction` server actions were missing rate limits. Because these are authenticated actions that modify the database, they require protection against automated spam.
**Learning:** Even internal or seemingly low-impact state modifications (like renaming a list or toggling its visibility) need rate limiting. Otherwise, an authenticated attacker can write a script to continuously invoke them, causing database resource exhaustion and potentially affecting application performance.
**Prevention:** Every server action that performs a database mutation (insert, update, delete) must be protected by a rate limit check, ideally keyed by the authenticated user's ID (`user.id`) to prevent cross-account starvation.
