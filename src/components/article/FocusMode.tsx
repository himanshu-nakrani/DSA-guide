...
export function FocusMode() {
  ...
  return (
    <button
      ...
      className="inline-flex min-h-[44px] items-center gap-1.5 px-2 text-note font-mono uppercase tracking-[0.06em] text-muted-foreground hover:text-foreground transition-colors rounded-md outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--links)]"
    >
      <span suppressHydrationWarning className="inline-flex items-center gap-1.5">
        {on ? <X className="h-3 w-3" /> : <Focus className="h-3 w-3" />}
        {on ? "Exit focus" : "Focus"}
        <kbd className="ml-1 px-1.5 py-0.5 rounded border border-border bg-muted text-micro">
          {on ? "esc" : "f"}
        </kbd>
      </span>
    </button>
