                  ...
                  <tr key={row}>
                    {Array.from({ length: COLS }, (_, col) => {
                      ...
                      return <td key={col} className="p-1">
                        <button type="button" disabled={!editing || isStart || isEnd} onClick={() => toggleObstacle(row, col)} aria-label={`${blocked ? "Remove" : "Add"} obstacle at row ${row}, column ${col}`} className="relative flex h-12 w-12 min-w-12 flex-col items-center justify-center border text-center transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--ink-blue)] disabled:cursor-default sm:h-16 sm:w-16 sm:min-w-16" style={{ borderColor: active ? PALETTE.c3 : blocked ? PALETTE.destructive : PALETTE.border, background: blocked ? "color-mix(in srgb, var(--ink-red) 12%, transparent)" : active ? "color-mix(in srgb, var(--ink-ochre) 17%, transparent)" : "var(--surface-1)" }}>
